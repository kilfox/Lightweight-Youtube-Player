...
try {
    ...
    if ($Runtime -eq 'win-x64') {
        Copy-Item -LiteralPath (Join-Path $repositoryRoot 'scripts\install.ps1') -Destination $resolvedOutput -Force
        Copy-Item -LiteralPath (Join-Path $repositoryRoot 'install.cmd') -Destination $resolvedOutput -Force
        ...
        Copy-Item -LiteralPath (Join-Path $repositoryRoot 'scripts\install.sh') -Destination $resolvedOutput -Force
    }
    ...
    Write-Host "Published to $resolvedOutput"
}
finally {
    Pop-Location
}
